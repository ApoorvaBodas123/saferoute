...
# Simplified fallback system (no model loading for faster startup)
...
# Import realistic route functions
import sys
import os
...
def calculate_fallback_route(start_lat, start_lon, end_lat, end_lon, strategy):
    ...
    try:
        # Use the realistic route function
...
def calculate_simple_route(start_lat, start_lon, end_lat, end_lon, strategy):
    ...
    return {
        ...
        "total_risk_score": risk_score,
            ...
            # Balanced route: Mix of main roads and shortcuts
            route_coordinates = [
            [start_lat, start_lon],
        ...
        distance = base_distance * 1.2  # 20% longer due to road following
        risk_score = 4.0  # Medium risk
        risk_level = "Medium"
        estimated_time = (distance / 25) * 60  # Moderate speed
        risk_score = 4.1  # Medium risk
        risk_level = 'Medium'
        estimated_time = (distance / 30) * 60  # Average speed
    ...
    return {
        ...
        'high_risk_segments': _identify_high_risk_segments(route_coordinates, risk_score),
        'is_fallback': True
    }

def _identify_high_risk_segments(route_coordinates, base_risk):
    """Identify high-risk segments in the route"""
    high_risk_segments = []
    
    for i in range(len(route_coordinates) - 1):
        # Simulate varying risk along the route
        segment_risk = base_risk + (i * 0.3)  # Risk increases along the route
